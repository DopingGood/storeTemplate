...
var ShowBtn = <HTMLElement>document.getElementsByClassName('arrival-button')[0];
if (ShowBtn) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function filterButtonClick() {
		var slctedSlct = <HTMLElement>this.parentElement, // filter div
			filterList = <HTMLCollection>slctedSlct.querySelectorAll('li'), // filter li
	...
	//	Filter sm-xs
	var filterSmBtn = <HTMLElement>document.getElementsByClassName('smf-btn')[0];
		var ddMenuSm = <HTMLDivElement>document.getElementsByClassName('smf-drop-menu')[0],
			filterBtnCar = <HTMLSpanElement>document.getElementsByClassName('filter-caret')[0],
			filterModal = <HTMLDivElement>document.getElementsByClassName('sm-filter-modal')[0],
			//	Drag variable
			smFilUl = <HTMLCollection>ddMenuSm.getElementsByClassName('smf-ul'),
			smFilterLinks = <HTMLCollection>ddMenuSm.querySelectorAll('.smf-ul a'),
			startMs = 0,
			ElOffset = 0,
			dragedEl,
			clickedEl;
		...
		for (var i = 0; i < smFilterLinks.length; i++) {
			...
			smFilterLinks[i].addEventListener('click', function() {
				var smSlctdLi = <HTMLLIElement>this.parentElement,
					smSlctRaw = <HTMLUListElement>this.closest('ul'),
					notSctdLi = <HTMLLIElement>smSlctRaw.getElementsByClassName("f-nSelectLi")[0],
					fashionName = <HTMLElement>ddMenuSm.querySelectorAll('fashion-li a')[0];
				// If clicked not selected
				if (this.classList.contains('f-nSelect') == true) {
					if (notSctdLi.classList.contains("naughtS") == false) {
						var slctdLi1 = <HTMLCollection>smSlctRaw.getElementsByClassName('flSelected');
				...
				// If clicked filter's selection
				else {
					var fashionUl = <HTMLUListElement>ddMenuSm.getElementsByClassName('smf-fashion')[0],
						typeUl = <HTMLUListElement>ddMenuSm.getElementsByClassName('smf-type')[0],
						colorUl = <HTMLUListElement>ddMenuSm.getElementsByClassName('smf-color')[0],
						brandUl = <HTMLUListElement>ddMenuSm.getElementsByClassName('smf-brand')[0],
						sizeUl = <HTMLUListElement>ddMenuSm.getElementsByClassName('smf-size')[0],
						priceUl = <HTMLUListElement>ddMenuSm.getElementsByClassName('smf-price')[0];	
					// Remove selection
					if (smSlctdLi.getAttribute('data-filter') == null) {
						smSlctdLi.classList.add('flSelected');
						smSlctdLi.setAttribute('data-filter', 'selected');
						var notSctdLi = <HTMLLIElement>smSlctRaw.getElementsByClassName("f-nSelectLi")[0];
...
var menuButton = <HTMLElement>document.getElementsByClassName('navbar-toggle')[0];
if (menuButton) {
